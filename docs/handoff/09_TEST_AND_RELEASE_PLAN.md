...
- `docs/release/manual-validation.json` validated by `python scripts/manual_release_evidence.py` for physical barcode camera, real receipt OCR, published image signature, and independent full review evidence. Published-image evidence must include the final `sha256:<digest>` and passing `cosign verify` command; independent review evidence must reference an existing review for the final commit with `decision=PASS`, `open_critical_high=0`, and `release_blocking_medium=0`.
