...
Published v1 release images must be signed outside this repository after the final image digest is known. Use a keyless or external-key `cosign sign` workflow controlled by the release operator, then record the SemVer `vX.Y.Z` release tag, image digest, signature identity, transparency-log URL or explicit unavailability reason, and verification command in the release notes. The final verification command recorded for release must verify the same published image reference, digest, and signature identity recorded in `docs/release/manual-validation.json`.
