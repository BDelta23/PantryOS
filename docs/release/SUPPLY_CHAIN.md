...
Published v1 release images must be signed outside this repository after the final image digest is known. Use a keyless or external-key `cosign sign` workflow controlled by the release operator, then record the SemVer `vX.Y.Z` release tag, SemVer-tagged digest-pinned published image reference, image digest, signature identity, transparency-log URL or explicit unavailability reason, and verification command in the release notes. The final verification command recorded for release must verify the same SemVer-tagged digest-pinned published image reference, digest, and signature identity recorded in `docs/release/manual-validation.json`, and must constrain the recorded identity as a `--certificate-identity` value or `--certificate-identity-regexp` pattern.
