...
Published v1 release images must be signed outside this repository after the final image digest is known. Use a keyless or external-key `cosign sign` workflow controlled by the release operator, then record the SemVer `vX.Y.Z` release tag, SemVer-tagged digest-pinned published image reference, image digest, signature identity, Rekor/Sigstore transparency-log URL or explicit unavailability reason, and exact `cosign verify` invocation in the release notes. The final verification invocation recorded for release must start with `cosign verify` and verify the same SemVer-tagged digest-pinned published image reference, digest, and signature identity recorded in `docs/release/manual-validation.json`, and must constrain the recorded identity as a `--certificate-identity` value or `--certificate-identity-regexp` pattern.
