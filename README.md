...
Browser camera barcode scanning requires a secure browser origin. `localhost` may work during development, but `http://<NAS-LAN-IP>:8765` is normally not allowed to use the camera on phones/tablets. Keep Home Assistant/API traffic on HTTP if desired, but use either a LAN HTTPS reverse proxy with `PANTRYOS_BROWSER_SECURE_COOKIES=true` or native PantryOS HTTPS with `PANTRYOS_TLS_CERT_FILE` and `PANTRYOS_TLS_KEY_FILE` pointing at a certificate/key mounted under `/data/tls`. The device used for scanning must trust that certificate, and the browser URL must be `https://<NAS-LAN-IP-or-name>:8765` for native HTTPS.
...
- If the browser says the camera cannot be used, check whether the address bar is using plain `http://<NAS-LAN-IP>:8765`. Most browsers block camera access on HTTP LAN origins; use a trusted HTTPS reverse proxy or native PantryOS HTTPS and open `https://<NAS-LAN-IP-or-name>:8765` from the scanning device.
