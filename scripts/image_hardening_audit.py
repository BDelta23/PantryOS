...
def static_checks() -> list[AuditCheck]:
    ...
    require(
        checks,
        "dockerfile-pinned-base-digest",
        bool(BASE_IMAGE_RE.search(dockerfile)),
        "Dockerfile pins the Python slim base image by sha256 digest.",
    )
    ...
    require(
        checks,
        "dockerfile-apt-cache-clean",
        "rm -rf /var/lib/apt/lists/*" in dockerfile,
        "APT package lists are removed in the same layer.",
    )
    require(
        checks,
        "dockerfile-dedicated-user",
        "--uid 10001" in dockerfile and "--gid 10001" in dockerfile,
        "Image defines dedicated pantryos UID/GID 10001.",
    )
    require(
        checks,
        "dockerfile-healthcheck",
        "HEALTHCHECK" in dockerfile and "/api/v1/health/ready" in dockerfile,
        "Image healthcheck probes readiness.",
    )
    require(
        checks,
        "dockerfile-packages-docker-contract",
        ".dockerignore" in dockerfile and "compose.yaml" in dockerfile,
        "Image packages Docker contract files for in-image audits.",
    )
    require(
        checks,
        "entrypoint-drops-privileges",
        "os.setuid(user.pw_uid)" in entrypoint and "os.setgid(user.pw_gid)" in entrypoint,
        "Entrypoint drops from root to pantryos user after data-volume repair.",
    )
    require(checks, "compose-read-only-root", "read_only: true" in compose, "Compose enables a read-only root filesystem.")
    require(
        checks,
        "compose-drop-capabilities",
        "cap_drop:" in compose and "- ALL" in compose,
        "Compose drops all Linux capabilities by default.",
    )
    require(
        checks,
        "compose-minimal-cap-add",
        all(item in compose for item in ("- CHOWN", "- SETGID", "- SETUID")),
        "Compose adds only startup capabilities needed for volume ownership repair and privilege drop.",
    )
    ...
    require(
        checks,
        "supply-chain-policy",
        (ROOT / "docs" / "release" / "SUPPLY_CHAIN.md").exists(),
        "Supply-chain policy documents lock, SBOM, and signing requirements.",
    )
    require(
        checks,
        "supply-chain-lock",
        (ROOT / "docs" / "release" / "container-image.lock.json").exists(),
        "Container image lock file is present.",
    )
    require(
        checks,
        "supply-chain-sbom",
        (ROOT / "docs" / "release" / "pantryos-image-sbom.spdx.json").exists(),
        "Container image SBOM is present.",
    )
...
def compose_checks(config: dict[str, Any]) -> list[AuditCheck]:
    ...
    require(
        checks,
        "compose-config-cap-drop",
        "ALL" in list(service.get("cap_drop") or []),
        "Rendered Compose config drops all capabilities by default.",
    )
    require(
        checks,
        "compose-config-minimal-cap-add",
        canonical_capabilities(service.get("cap_add")) == ["CHOWN", "SETGID", "SETUID"],
        "Rendered Compose config adds only CHOWN, SETGID, and SETUID.",
    )
    require(checks, "compose-config-pids-limit", int(service.get("pids_limit") or 0) == 256, "Rendered Compose config sets pids_limit=256.")
    require(
        checks,
        "compose-config-no-new-privileges",
        "no-new-privileges:true" in list(service.get("security_opt") or []),
        "Rendered Compose config has no-new-privileges.",
    )
    tmpfs = service.get("tmpfs") or []
    require(
        checks,
        "compose-config-tmpfs",
        any(str(item).startswith("/tmp:") and "size=64m" in str(item) for item in tmpfs),
        "Rendered Compose config has bounded /tmp tmpfs.",
    )
    volumes = [
        str(item.get("source", "")) + ":" + str(item.get("target", "")) if isinstance(item, dict) else str(item)
...
def live_checks(image: str, container: str) -> list[AuditCheck]:
    ...
    require(
        checks,
        "image-no-runtime-token",
        not any(item.startswith("PANTRYOS_API_TOKEN=") for item in image_env),
        "Image metadata does not bake a PantryOS API token.",
    )
    require(
        checks,
        "image-entrypoint",
        image_config.get("Config", {}).get("Entrypoint") == ["python", "scripts/docker_entrypoint.py"],
        "Image entrypoint uses the privilege-drop wrapper.",
    )
    require(checks, "image-healthcheck", bool(image_config.get("Config", {}).get("Healthcheck")), "Image metadata includes a healthcheck.")
    require(
        checks,
        "container-healthy",
        state.get("Status") == "running" and state.get("Health", {}).get("Status") == "healthy",
    ...
    require(
        ...
        "no-new-privileges:true" in list(host_config.get("SecurityOpt") or []),
        "Container runs with no-new-privileges.",
    )
    require(
        checks,
        "container-cap-drop-all",
        "ALL" in list(host_config.get("CapDrop") or []),
        "Container drops all Linux capabilities by default.",
    )
    require(
        checks,
        "container-minimal-cap-add",
        canonical_capabilities(host_config.get("CapAdd")) == ["CHOWN", "SETGID", "SETUID"],
        "Container adds only startup capabilities needed for chown/setuid.",
    )
    ...
    require(
        checks,
        "process-non-root",
        status.get("Uid", "").split()[0] == "10001" and status.get("Gid", "").split()[0] == "10001",
        "PantryOS process runs as UID/GID 10001.",
    )
    require(checks, "process-no-new-privileges", status.get("NoNewPrivs") == "1", "PantryOS process has NoNewPrivs=1.")
    require(
        checks,
        "process-no-effective-capabilities",
        status.get("CapEff") in {"0000000000000000", "0"},
        "PantryOS process has no effective Linux capabilities after privilege drop.",
    )
    return checks
